...
| Delete generated images after a configured delay      | Local expiry is implemented; optional operator deletion integrates the documented v1 generation-delete endpoint, pending live verification |
...
unverified and require live confirmation before launch. As checked on 2026-09-26,
the [v1 generation-delete contract](https://docs.leonardo.ai/v1.0/reference/deletegenerationbyid)
does expose `DELETE /generations/{id}`. Its applicability to the app's v2
generations and remote/CDN erasure still need live confirmation. A Flare application-owned `URL`
...
| `generationId`                   | v2 `POST /generations`                                              | Correlates completion, persistence, and output | v1 `DELETE /generations/{id}` is documented; v2-output compatibility and erasure remain unverified                           |
...
durable cleanup job. Leonardo's FAQ says API-generated images do not expire.
The v1 generation-delete endpoint, checked on 2026-09-26, returns a nullable
`delete_generations_by_pk.id`. The optional overview deletion workflow validates
a matching ID, keeps uncertain outcomes for operator recovery, and retries only
explicit HTTP 429 rejections automatically. Transport failures, null/mismatched
responses and lost acknowledgements do not prove deletion. Live verification
must establish compatibility with the current v2 output and repeated-delete
semantics before real-photo use. `public: false` controls visibility; it is not
deletion, and an API acknowledgement is not proof of backup/CDN erasure.
...
image deletion remains unverified for the current model output and may still
