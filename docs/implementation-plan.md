...
dimensions, and a `MID` uploaded-image strength; all other themes retain the
Sunburst profile. The prior Dynamic and Illustration profiles remain available
only to finish or reconcile durable generations that already reference them.
The profile is internal to the server, generation row, and worker: sessions,
public themes, photo responses, and browser requests never contain it. New
model profiles require documented schema and cost verification before a theme
can select them, while existing profiles remain available for queued and
retried jobs. Earlier Flare submissions
